...
function registerTools(server: McpServer, api: AxiosInstance) {
  ...
  server.tool(
    ...
    { candidates: z.array(z.string()), tlds: z.array(z.string()).optional(), use_case: z.string().optional(), deep_check_top_n: z.number().optional() },
    async ({ candidates, tlds, use_case, deep_check_top_n }) => {
      try {
  ...
  server.tool(
    "netintel_openai",
    "Call OpenAI's gpt-4o via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.10 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output. Search terms: OpenAI…",
  ...
  server.tool(
    ...
    "OpenAI-compatible chat completions gateway — standard /v1/chat/completions path and request shape, model chosen in the body (gpt-4o, gpt-4.1, gpt-5.6-luna, minis, nanos). Flat $0.10 per call in USDC via x402, no OpenAI account or API key…",
    { model: z.string(), messages: z.array(z.any()), max_tokens: z.number().optional() },
  ...
  server.tool(
    ...
    { symbol: z.string(), vs: z.string().optional() },
    async ({ symbol, vs }) => {
      try {
  ...
  server.tool(
    ...
    { city: z.string().optional(), latitude: z.number().optional(), longitude: z.number().optional(), units: z.string().optional() },
    async ({ city, latitude, longitude, units }) => {
      try {
  ...
  server.tool(
    ...
    { city: z.string().optional(), latitude: z.number().optional(), longitude: z.number().optional(), days: z.number().optional(), hourly: z.boolean().optional(), units: z.string().optional() },
    async ({ city, latitude, longitude, days, hourly, units }) => {
      try {
  ...
  server.tool(
    ...
    { query: z.string().optional(), limit: z.number().optional(), status: z.string().optional() },
  ...
  server.tool(
    ...
    "Verify an email address before you send: syntax + MX + disposable/role/free-provider detection in one call, boolean-first (deliverable, safe_to_send) with a machine-readable reason. DNS-level check only — does not confirm the individual…",
    { email: z.string() },
    async ({ email }) => {
      try {
  ...
  server.tool(
    "netintel_exa_search",
    "exaSearchRouteConfig.description",
    { query: z.string(), num_results: z.number().optional(), type: z.string().optional(), category: z.string().optional(), include_domains: z.array(z.string()).optional(), start_published_date: z.string().optional(), end_published_date: z.string().optional(), snippets: z.string().optional() },
    async ({ query, num_results, type, category, include_domains, start_published_date, end_published_date, snippets }) => {
      try {
  ...
  server.tool(
    "netintel_web_search",
    "webSearchRouteConfig.description",
    { query: z.string(), num_results: z.number().optional(), type: z.string().optional(), category: z.string().optional(), include_domains: z.array(z.string()).optional(), start_published_date: z.string().optional(), end_published_date: z.string().optional(), snippets: z.string().optional() },
    async ({ query, num_results, type, category, include_domains, start_published_date, end_published_date, snippets }) => {
      try {
  ...
  server.tool(
    ...
    { urls: z.array(z.string()), max_characters: z.number().optional(), livecrawl: z.string().optional() },
    async ({ urls, max_characters, livecrawl }) => {
      try {
  ...
  server.tool(
    ...
    { query: z.string(), include_citation_text: z.boolean().optional() },
    async ({ query, include_citation_text }) => {
      try {
